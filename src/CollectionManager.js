            ...
            const [placenames, mapDefs] = await Promise.all([
                window.electronAPI.loadFromJson(templatePath, collection.placeNamesFile)
                    .catch(err => {
                        ...
                        throw new Error(`Could not load required file: ${collection.placeNamesFile}. Please check that your template folder is configured correctly.`);
                    }),
                window.electronAPI.loadFromJson(templatePath, collection.mapDefsFile)
                    .catch(err => {
                        ...
                        throw new Error(`Could not load required file: ${collection.mapDefsFile}. Please check that your template folder is configured correctly.`);
                    })
            ]);
