...
export default function SettingsModal ({open, onClose, labelScale, setLabelScale, lang, setLang, projectFolder, handleSelectProjectFolder, templateFolder, handleSelectTemplateFolder}) {
  if (!open) return null;
  return (
    <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.2)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="modal-content" style={{ background: '#fff', borderRadius: 8, padding: 24, minWidth: 320, maxWidth: 600, boxShadow: '0 2px 16px rgba(0,0,0,0.2)' }}>
        ...
        {/* Template folder selector UI */}
        <div style={{ padding: '6px 12px', marginBottom: 16, background: '#f0f0f0', borderBottom: '1px solid #ccc', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontWeight: 'bold' }}>Template Folder:</span>
          <span style={{ color: templateFolder ? '#333' : '#888', fontFamily: 'monospace', fontSize: 13, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{templateFolder || '(using default)'}</span>
          <button onClick={handleSelectTemplateFolder} style={{ padding: '2px 10px', borderRadius: 4, border: '1px solid #888', background: '#e3f2fd', cursor: 'pointer' }}>Change…</button>
        </div>
        
        {/* Project folder selector UI - commented out but preserved */}
        {/*
