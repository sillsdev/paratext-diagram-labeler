@echo off
echo Building complete Scripture Map Labeler Release Package...
echo -----------------------------------------------------
...
if exist "dist\win-unpacked\Scripture Map Labeler.exe" (
    ...
    call npx electron-builder --win portable --prepackaged dist\win-unpacked --config.win.target=portable --config.win.artifactName="Scripture Map Labeler Portable ${version}.${ext}" --config.asar=false
